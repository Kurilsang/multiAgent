...
- 下一步：MCP 市场剩余票（#39 文档收口）、多 Agent 协作（状态机已留后门：新增状态与迁移边）、ClawHub 适配器与已验证源免确认白名单、`/` 弹层混入在线目录、多会话持久化、网关逻辑（路由/鉴权/审计）
