...
- **实现裁定（21 号工单，较 SPEC-0001 的闸门适配）**：目录包安装对第三方 frontmatter 采用**未知字段一律丢弃而非拒绝**——字段值根本不进上下文，注入面不变；生态常见的 `allowed-tools` 等字段丢弃、工具依赖留空（外部工具名与本注册表不通，用户可在包内补 `tools` 声明）。若按严格白名单拒绝，真实生态包（如 anthropics/skills）会被整体挡在门外。自有技能（create_skill / 手装）仍走严格解析
